...
public class TestForNettyTransporterServer {
    ...
    public void testForServer() throws InterruptedException {
        ...
        ServantAdapterConfig servantAdapterConfig = ServantAdapterConfig.makeServantAdapterConfig(endpoint,
                "servantConfig", null);
        NettyTransporterServer nettyTransporterServer = new NettyTransporterServer(servantAdapterConfig,
                channelHandlerer);
        try {
    ...
    public void testForAdapter() throws Throwable {
        ...
        // Wait briefly to ensure server is up, then shut it down
        Thread.sleep(500);

        // Test is successful if server binds without exception
        // Shutdown the server to avoid hanging
        servantAdapter.stop();
    }
    ...
    public static class MonitorQuerySeravntImpl implements MonitorQueryServant {
        ...
        public int query(MonitorQueryReq req, Holder<MonitorQueryRsp> rsp) {
            ...
            rsp.getValue().setRetThreads(new ArrayList<>());
            rsp.getValue().setMsg("'");
